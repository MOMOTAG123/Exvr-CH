...
class VideoWindow(QMainWindow):
    def __init__(self):
        ...
        self.flip_x_checkbox = QCheckBox("沿x轴镜像", self)
        ...
        self.flip_y_checkbox = QCheckBox("沿y轴镜像", self)
        ...
        self.ip_camera_url_input.setPlaceholderText("输入网络摄像头IP地址")
        ...
        self.priority_selection.addItems(["空闲运行", "低级运行", "正常运行", "中等运行", "高级运行", "实时运行"])
        ...
        self.toggle_button = QPushButton("启动捕捉", self)
        ...
        self.show_frame_button = QPushButton("显示摄像头", self)
        ...
        self.only_ingame_checkbox = QCheckBox("自动打开游戏", self)
        ...
        self.only_ingame_checkbox.setToolTip("目前，这仅适用于热键和鼠标输入，不适用于头部移动")
        ...
        self.checkbox2 = QCheckBox("脸", self)
        ...
        self.checkbox4 = QCheckBox("手臂", self)
        ...
        self.checkbox5 = QCheckBox("姿势", self)
        ...
        self.checkbox6 = QCheckBox("是否启用丢追时手臂自动下垂", self)
        ...
        self.checkbox7 = QCheckBox("是否启用手势控制", self)
        ...
        self.controller_checkbox1 = QCheckBox("左手控制器开关", self)
        ...
        self.controller_checkbox2 = QCheckBox("右手控制器开关", self)
        ...
        self.mouse_checkbox = QCheckBox("鼠标控制", self)
        ...
        self.save_config_button = QPushButton("保存UI设置的Config信息", self)
    ...
    def face_dialog(self):
        ...
        self.dialog.setWindowTitle("脸部设置")
        ...
        form_layout.addWidget(QLabel("混合变形"), 0, 0)
        form_layout.addWidget(QLabel("数值"), 0, 1)
        form_layout.addWidget(QLabel("偏移量"), 0, 2)
        form_layout.addWidget(QLabel("权重"), 0, 3)
        form_layout.addWidget(QLabel("最大值"), 0, 4)
        form_layout.addWidget(QLabel("启用/停止"), 0, 5)
        ...
        self.save_config_button = QPushButton("保存设置", self.dialog)
    ...
    def set_process_priority(self):
        ...
        priority_classes = {
        "空闲运行": 0x00000040,        # IDLE_PRIORITY_CLASS
        "低级运行": 0x00004000,       # BELOW_NORMAL_PRIORITY_CLASS
        "正常运行": 0x00000020,       # NORMAL_PRIORITY_CLASS
        "中等运行": 0x00008000,       # ABOVE_NORMAL_PRIORITY_CLASS
        "高级运行": 0x00000080,       # HIGH_PRIORITY_CLASS
        "实时运行": 0x00000100        # REALTIME_PRIORITY_CLASS
    }
        # Check if the index is valid
        if priority_key not in priority_classes:
            self.display_message("Error","优先级索引无效")
            return False
    ...
    def install_function(self):
        ...
        if check_steamvr_path is not None:
            ...
            self.steamvr_status_label.setText("SteamVR 没有安装")
            self.steamvr_status_label.setStyleSheet("color: red; font-weight: bold;")
        if self.install_state:
            # Uninstall process
            ...
            for driver in drivers_to_remove:
                ...
                if os.path.exists(dir_path):
                    error_occurred = True
            if error_occurred:
                self.display_message("Error", "SteamVR正在运行，请关闭SteamVR并重试。")
                return
            try:
                os.remove(dll_path)
            except PermissionError:
                self.display_message("Error", "VRCFT正在运行，请关闭VRCFT并重试。")
                return
            ...
            self.install_button.setText("卸载驱动")
            self.install_button.setStyleSheet("")
    ...
    def toggle_camera(self):
        ...
        if self.video_thread and self.video_thread.isRunning():
            stop_hotkeys()
            self.toggle_button.setText("启动捕捉")
            self.toggle_button.setStyleSheet(
                "QPushButton { background-color: green; color: white; }"
            )
            self.thread_stopped()
            ...
            apply_hotkeys()
            self.toggle_button.setText("停止捕捉")
            self.toggle_button.setStyleSheet(
        ...
        self.show_frame_button.setText("显示摄像头")
    ...
    def toggle_video_display(self):
        if self.video_thread:
            if self.video_thread.show_image:
                ...
                self.show_frame_button.setText("隐藏摄像头")
        else:
            self.show_frame_button.setText("显示摄像头")
